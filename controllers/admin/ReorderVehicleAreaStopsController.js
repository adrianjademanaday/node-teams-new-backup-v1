...
/**
 * 
 * Shows the main page for Reorder Vehicle AreaStops task
 * 
 */

exports.show = function(req, res) {
	Area.find({}, null, {sort: {name: 1}}, function(err, areas) {
		res.render('admin/reorderVehicleAreaStops/show', {
			areas: areas,
	 		areaId: null,
	 		message: null	 		
	 	});	
...
/**
 * 
 * Renames all Passenger AreaStops by number and sorts them by that.
 * 
 */
...
exports.reorder = function(req, res) {
	...
	fetchArea(areaId)
	.then(function(area) {
		VehicleAreaStop.find({area: area}, null, {sort: {name: 1}}, function(err, vehicleAreaStops) {
			vehicleAreaStops.forEach(function(as) {
		...
		Area.find({}, null, {sort: {name: 1}}, function(err, areas) {
			res.render('admin/reorderVehicleAreaStops/show', {
				areas: areas,
		 		areaId: null,
		 		message: 'Reordered Vehicle Area Stops'
		 	});	
		});
	});
};
